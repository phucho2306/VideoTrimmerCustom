  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onLongPressStart: (details) {
        ...
        _dragType = EditorDragType.center;
        oldDx = details.localPosition.dx;
      },
      onLongPressMoveUpdate: (details) {
        if (!_allowDrag) return;
        _circleSize = widget.circleSizeOnDrag;
        if (_isStartNew) {
          oldDx = details.offsetFromOrigin.dx;
          _isStartNew = false;
        }
        double newDx = details.offsetFromOrigin.dx - oldDx;

        if ((_startPos.dx + newDx >= 0) && (_endPos.dx + newDx <= _thumbnailViewerW)) {
          oldDx = details.offsetFromOrigin.dx;
          _startPos += Offset(newDx, 0);
          _endPos += Offset(newDx, 0);
